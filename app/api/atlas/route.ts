...
import { buildAtlasSystemPrompt } from "@/lib/atlas/persona";
import { buildAtlasTools, runAtlasTool, ATLAS_TOOL_NAMES } from "@/lib/atlas/tools";
import type { AtlasToolContext } from "@/lib/atlas/tools";
...
export async function POST(request: Request) {
  ...
  const parsed = await parseJsonBody(request, AtlasSchema, "atlas");
  ...
  if (isAgentMockModeEnabled()) {
    return NextResponse.json(buildMockAgentResponse("atlas", message, body.conversationId ?? undefined, profile.role), {
      headers: { "Cache-Control": "no-store" },
    });
  }
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return NextResponse.json({ error: "Atlas isn't configured yet. Add ANTHROPIC_API_KEY in Vercel." }, { status: 503 });
  ...
  if (!conversationId) {
    const convInsert: Database["public"]["Tables"]["ai_conversations"]["Insert"] = { org_id: profile.org_id, user_id: profile.id, title: `Atlas — ${message.slice(0, 48)}` };
  ...
  const systemPrompt = [buildAtlasSystemPrompt(profile.role, profile.full_name), UNTRUSTED_DATA_SYSTEM_PROMPT].join("\n\n");
  const tools = buildAtlasTools(profile.role);
  const toolCtx: AtlasToolContext = { supabase, profile };
  ...
  try {
    for (let round = 0; ; round++) {
      ...
      if (!res.ok) {
        ...
        console.error("Anthropic API error (atlas)", res.status, detail);
        return NextResponse.json({ error: "Atlas is temporarily unavailable." }, { status: 502 });
      }
      ...
      if (allowTools && data.stop_reason === "tool_use") {
        ...
        const toolResults = await Promise.all(
          toolUses.map(async (block) => {
            ...
            else if ((ATLAS_TOOL_NAMES as Set<string>).has(block.name)) {
              result = await runAtlasTool(block.name as any, block.input, toolCtx);
            } else result = { error: `Unknown tool: ${block.name}` };
            if (block.name === "propose_atlas_capture" && (result as { ok?: boolean }).ok) { revalidate.add("/approvals"); revalidate.add("/atlas"); }
    ...
    console.error("Atlas request failed", err);
    return NextResponse.json({ error: "Atlas is temporarily unavailable." }, { status: 502 });
  }
  ...
  if (!filtered.safe) {
    void recordSecurityEvent(supabase, { orgId: profile.org_id, userId: profile.id, eventType: "output_redacted", severity: "critical", detail: { agent: "atlas", findings: filtered.findings.map((f) => ({ kind: f.kind, label: f.label })) } });
  }
  void recordAiUsage(supabase, { orgId: profile.org_id, userId: profile.id, feature: "atlas", model, inputTokens: totalInputTokens, outputTokens: totalOutputTokens });
