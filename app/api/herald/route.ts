...
import { buildHeraldSystemPrompt } from "@/lib/herald/persona";
import { buildHeraldTools, runHeraldTool, HERALD_TOOL_NAMES } from "@/lib/herald/tools";
import type { HeraldToolContext } from "@/lib/herald/tools";
...
export async function POST(request: Request) {
  ...
  const parsed = await parseJsonBody(request, HeraldSchema, "herald");
  ...
  if (isAgentMockModeEnabled()) {
    return NextResponse.json(buildMockAgentResponse("herald", message, body.conversationId ?? undefined, profile.role), {
      headers: { "Cache-Control": "no-store" },
    });
  }
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return NextResponse.json({ error: "Herald isn't configured yet. Add ANTHROPIC_API_KEY in Vercel." }, { status: 503 });
  ...
  if (!conversationId) {
    const convInsert: Database["public"]["Tables"]["ai_conversations"]["Insert"] = { org_id: profile.org_id, user_id: profile.id, title: `Herald — ${message.slice(0, 48)}` };
  ...
  const systemPrompt = [buildHeraldSystemPrompt(profile.role, profile.full_name), UNTRUSTED_DATA_SYSTEM_PROMPT].join("\n\n");
  const tools = buildHeraldTools(profile.role);
  const toolCtx: HeraldToolContext = { supabase, profile };
  ...
  try {
    for (let round = 0; ; round++) {
      ...
      if (!res.ok) {
        ...
        console.error("Anthropic API error (herald)", res.status, detail);
        return NextResponse.json({ error: "Herald is temporarily unavailable." }, { status: 502 });
      }
      ...
      if (allowTools && data.stop_reason === "tool_use") {
        ...
        const toolResults = await Promise.all(
          toolUses.map(async (block) => {
            ...
            else if ((HERALD_TOOL_NAMES as Set<string>).has(block.name)) {
              result = await runHeraldTool(block.name as any, block.input, toolCtx);
            } else result = { error: `Unknown tool: ${block.name}` };
            if (block.name === "propose_herald_send" && (result as { ok?: boolean }).ok) { revalidate.add("/approvals"); revalidate.add("/herald"); }
    ...
    console.error("Herald request failed", err);
    return NextResponse.json({ error: "Herald is temporarily unavailable." }, { status: 502 });
  }
  ...
  if (!filtered.safe) {
    void recordSecurityEvent(supabase, { orgId: profile.org_id, userId: profile.id, eventType: "output_redacted", severity: "critical", detail: { agent: "herald", findings: filtered.findings.map((f) => ({ kind: f.kind, label: f.label })) } });
  }
  void recordAiUsage(supabase, { orgId: profile.org_id, userId: profile.id, feature: "herald", model, inputTokens: totalInputTokens, outputTokens: totalOutputTokens });
