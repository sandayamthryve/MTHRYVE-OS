...
import { buildProspectorSystemPrompt } from "@/lib/prospector/persona";
import { buildProspectorTools, runProspectorTool, PROSPECTOR_TOOL_NAMES } from "@/lib/prospector/tools";
import type { ProspectorToolContext } from "@/lib/prospector/tools";
...
export async function POST(request: Request) {
  ...
  const parsed = await parseJsonBody(request, ProspectorSchema, "prospector");
  ...
  if (isAgentMockModeEnabled()) {
    return NextResponse.json(buildMockAgentResponse("prospector", message, body.conversationId ?? undefined, profile.role), {
      headers: { "Cache-Control": "no-store" },
    });
  }
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return NextResponse.json({ error: "Prospector isn't configured yet. Add ANTHROPIC_API_KEY in Vercel." }, { status: 503 });
  ...
  if (!conversationId) {
    const convInsert: Database["public"]["Tables"]["ai_conversations"]["Insert"] = { org_id: profile.org_id, user_id: profile.id, title: `Prospector — ${message.slice(0, 48)}` };
  ...
  const systemPrompt = [buildProspectorSystemPrompt(profile.role, profile.full_name), UNTRUSTED_DATA_SYSTEM_PROMPT].join("\n\n");
  const tools = buildProspectorTools(profile.role);
  const toolCtx: ProspectorToolContext = { supabase, profile };
  ...
  try {
    for (let round = 0; ; round++) {
      ...
      if (!res.ok) {
        ...
        console.error("Anthropic API error (prospector)", res.status, detail);
        return NextResponse.json({ error: "Prospector is temporarily unavailable." }, { status: 502 });
      }
      ...
      if (allowTools && data.stop_reason === "tool_use") {
        ...
        const toolResults = await Promise.all(
          toolUses.map(async (block) => {
            ...
            else if ((PROSPECTOR_TOOL_NAMES as Set<string>).has(block.name)) {
              result = await runProspectorTool(block.name as any, block.input, toolCtx);
            } else result = { error: `Unknown tool: ${block.name}` };
            if (block.name === "propose_prospect_task" && (result as { ok?: boolean }).ok) { revalidate.add("/approvals"); revalidate.add("/prospector"); }
            if (block.name === "find_prospects" && (result as { ok?: boolean }).ok) revalidate.add("/prospector");
    ...
    console.error("Prospector request failed", err);
    return NextResponse.json({ error: "Prospector is temporarily unavailable." }, { status: 502 });
  }
  ...
  if (!filtered.safe) {
    void recordSecurityEvent(supabase, { orgId: profile.org_id, userId: profile.id, eventType: "output_redacted", severity: "critical", detail: { agent: "prospector", findings: filtered.findings.map((f) => ({ kind: f.kind, label: f.label })) } });
  }
  void recordAiUsage(supabase, { orgId: profile.org_id, userId: profile.id, feature: "prospector", model, inputTokens: totalInputTokens, outputTokens: totalOutputTokens });
