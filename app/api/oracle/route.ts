...
import { buildOracleSystemPrompt } from "@/lib/oracle/persona";
import { buildOracleTools, runOracleTool, ORACLE_TOOL_NAMES } from "@/lib/oracle/tools";
import type { OracleToolContext } from "@/lib/oracle/tools";
...
export async function POST(request: Request) {
  ...
  const parsed = await parseJsonBody(request, OracleSchema, "oracle");
  ...
  if (isAgentMockModeEnabled()) {
    return NextResponse.json(buildMockAgentResponse("oracle", message, body.conversationId ?? undefined, profile.role), {
      headers: { "Cache-Control": "no-store" },
    });
  }
  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return NextResponse.json({ error: "Oracle isn't configured yet. Add ANTHROPIC_API_KEY in Vercel." }, { status: 503 });
  ...
  if (!conversationId) {
    const convInsert: Database["public"]["Tables"]["ai_conversations"]["Insert"] = { org_id: profile.org_id, user_id: profile.id, title: `Oracle — ${message.slice(0, 48)}` };
  ...
  const systemPrompt = [buildOracleSystemPrompt(profile.role, profile.full_name), UNTRUSTED_DATA_SYSTEM_PROMPT].join("\n\n");
  const tools = buildOracleTools(profile.role);
  const toolCtx: OracleToolContext = { supabase, profile };
  ...
  try {
    for (let round = 0; ; round++) {
      ...
      if (!res.ok) {
        ...
        console.error("Anthropic API error (oracle)", res.status, detail);
      ...
      if (allowTools && data.stop_reason === "tool_use") {
        ...
        const toolResults = await Promise.all(
          toolUses.map(async (block) => {
            ...
            else if ((ORACLE_TOOL_NAMES as Set<string>).has(block.name)) {
              result = await runOracleTool(block.name as any, block.input, toolCtx);
            } else result = { error: `Unknown tool: ${block.name}` };
            if (block.name === "propose_oracle_action" && (result as { ok?: boolean }).ok) { revalidate.add("/approvals"); revalidate.add("/oracle"); }
  ...
  if (!filtered.safe) {
    void recordSecurityEvent(supabase, { orgId: profile.org_id, userId: profile.id, eventType: "output_redacted", severity: "critical", detail: { agent: "oracle", findings: filtered.findings.map((f) => ({ kind: f.kind, label: f.label })) } });
  }
  void recordAiUsage(supabase, { orgId: profile.org_id, userId: profile.id, feature: "oracle", model, inputTokens: totalInputTokens, outputTokens: totalOutputTokens });
