...
export type ImportProjectsState = { imported: number; skipped: string[] } | null;

function SubmitButton() {
  const { pending } = useFormStatus();
  return <button type="submit" disabled={pending} className="rounded-md bg-teal-500 px-4 py-2 text-sm font-semibold text-charcoal-950 hover:bg-teal-400 disabled:opacity-60">{pending ? "Importing…" : "Import Projects (CSV)"}</button>;
}

export function ImportProjectsControl({ action }: { action: (prev: ImportProjectsState, formData: FormData) => Promise<ImportProjectsState> }) {
  const [state, formAction] = useFormState(action, null);
  const [toastMounted, setToastMounted] = useState(false);
  const [toastVisible, setToastVisible] = useState(false);
  const formRef = useRef<HTMLFormElement>(null);
  const helper = "Bulk-add projects from a spreadsheet. Rows missing a name — or naming an owner_email that matches no user — are skipped and reported. Department is matched by name; status defaults to Active when blank.";
