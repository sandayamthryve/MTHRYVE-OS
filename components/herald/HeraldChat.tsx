...
const SUGGESTIONS = [
  "List outreach queue",
  "Get outreach target Alex",
  "Draft message for Alex",
  "Propose send to Alex",
];

export function HeraldChat({ canFile }: { canFile: boolean }) {
  ...
  function send(text: string) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          setError(data.error ?? "Herald is unavailable.");
          return;
        }
        ...
        setError("Herald is unavailable. Please try again.");
      }
    });
  }
    ...
    <div className="flex h-[32rem] flex-col rounded-xl border border-charcoal-700/60 bg-charcoal-900 shadow-elevate">
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 ? (
          <div className="text-sm text-ink-muted">
            <p className="mb-3">
              I&apos;m Herald — your outreach drafting agent. Ask for the outreach queue, a lead or creator, or a follow-up draft. I propose; leadership approves before anyone is contacted.
            </p>
            ...
            {!canFile && (
              <p className="mt-3 text-[11px] text-amber-300">
                Your role can draft but can&apos;t file sends (leadership only).
              </p>
            )}
          </div>
        ) : (
        ...
        {pending && <p className="text-xs text-ink-dim">Herald is listening…</p>}
      </div>
      ...
      >
        <input
          ...
          placeholder="Ask Herald — e.g. draft a follow-up for Alex…"
          disabled={pending}
