...
const SUGGESTIONS = [
  "List prospect queue",
  "Find prospects from CSV",
  "Get prospect context for Alex",
  "Propose qualification for Alex as HOT",
];

export function ProspectorChat({ canFile }: { canFile: boolean }) {
  ...
  function send(text: string) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!res.ok) {
          setError(data.error ?? "Prospector is unavailable.");
          return;
        }
        ...
        setError("Prospector is unavailable. Please try again.");
      }
    });
  }
    ...
    <div className="flex h-[32rem] flex-col rounded-xl border border-charcoal-700/60 bg-charcoal-900 shadow-elevate">
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto p-4">
        {messages.length === 0 ? (
          <div className="text-sm text-ink-muted">
            <p className="mb-3">
              I&apos;m Prospector — your wellbeing and people-care agent. Ask for your care summary, the team pulse, a probation check-in, or HR policy — I propose, HR approves before anyone is contacted.
            </p>
            ...
            {!canFile && (
              <p className="mt-3 text-[11px] text-amber-300">
                Your role can scout but can&apos;t file qualification tasks (leadership only).
              </p>
            )}
          </div>
        ) : (
        ...
        {pending && <p className="text-xs text-ink-dim">Prospector is listening…</p>}
      </div>
      ...
      >
        <input
          ...
          placeholder="Ask Prospector — e.g. log my mood 4, team pulse, check on Alex…"
          disabled={pending}
